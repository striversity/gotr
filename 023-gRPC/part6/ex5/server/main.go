...
func myUnaryServerInterceptor(ctx context.Context, req interface{},
	...
	idx := strings.Index(info.FullMethod, "/model.MyMathService/Mod")
	if idx == 0 {
		if ok, err := authorized(ctx, info.FullMethod); !ok {
			return nil, err
		}
	}

	idx = strings.Index(info.FullMethod, "/model.MyMathService/")
	if idx == 0 {
		if ok, err := authorized2(ctx, info.FullMethod); !ok {
			return nil, err
		}
	}
